  ...
  <div class="main-frame" :style="bannerBgImage()">
    <div class="top-container">
      ...
      <div class="buttons-container col-3">
        <div class="frame-time">
          <span v-if="!voted"
            >1 month ago in {{ celebArrayRes.category }}
          </span>
        ...
        <div class="frame-buttons">
          <button
            ...
            :class="
              btnPositiveAct
                ? 'vote-btn vote-btn-up active-btn'
                : 'vote-btn vote-btn-up'
            "
            @click="activePositive"
          >
            <img src="../../assets/img/thumbs-up.svg" alt="" />
          </button>
          <button
            ...
            :class="
              btnNegativeAct
                ? 'vote-btn vote-btn-down active-btn'
                : 'vote-btn vote-btn-down'
            "
            @click="activeNegative"
          >
            <img src="../../assets/img/thumbs-down.svg" alt="" />
          </button>
          <button v-if="!voted" class="submit-vote-btn" @click="sendVote">
            Vote Now
          </button>
          <button v-else class="submit-vote-btn" @click="reVote">
            Vote Again
          </button>
        </div>
      </div>
    ...
    <div>
      <div class="votes-container">
        <div
          class="thumbs-up-bar votes-bar"
          :style="'width:' + positiveResult + '%'"
        >
          <span>{{ positiveResult }} %</span>
        </div>
        <div
          class="thumbs-down-bar votes-bar"
          :style="'width:' + negativeResult + '%'"
        >
          <span>{{ negativeResult }} %</span>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    bannerBgImage() {
      return (
        "background-image: linear-gradient(90deg, rgba(0,0,0,0) 0%, rgba(119,119,119,1) 16%, rgba(88,88,88,1) 100%), url(/img/" +
        this.celebrityArr.picture +
        ".jpg)"
      );
    },
    reVote() {
  ...
  computed: {
    ...
    positiveResult() {
      let positive =
        (this.celebArrayRes.votes.positive /
          (this.celebArrayRes.votes.positive +
            this.celebArrayRes.votes.negative)) *
        100;
